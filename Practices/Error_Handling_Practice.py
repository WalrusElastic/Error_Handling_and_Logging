...
# Exercise 2: Using context managers (with)
# Problem: The function below should read a text file and return the the integer in the file, but fails without closing the file when it has non integer content.
# Implement this using a `with` statement to open the file. Handle the case where the file's contents is not an integer and return -1.

def get_integer_from_file(filename: str) -> int:
    f = open(filename, "r")
    content = f.read()
    number = int(content.strip())
    return number

test_cases = [("test_1.txt", 67), ("test_2.txt", -1)]

for test_file, expected in test_cases:
    result = get_integer_from_file(test_file)
    print(f"get_integer_from_file('{test_file}') = {result}, expected = {expected}\n")
...
test_cases = [(["1", "2"], 3), (["1.5", "2.5"], None),(["1", "two"], None), (["1"], None), (["1", 2], 3)]
# for i in range(len(test_cases)):
...
test_cases = [("80,75,90", 2, 85.0), ("80,75,90", 5, None), ("80,75,90", -1, None), ("80,abc,90", 2, None)]
